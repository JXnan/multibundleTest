...
function getModuleIdByIndex(projectRootPath, path, entry, isBuz) {
  ...
  if (curModuleId == -100) { // 初始化基础包id
    curModuleId = - 1;//基础包的Module都是从0开始的
  }

  if (baseMappingPath == null) { // 基础包路径
    baseMappingPath = __dirname + pathSep + "platformMap.json";
  }
  if (baseModuleIdMap.length == 0) { // 基础mapinfo
    if (fs.existsSync(baseMappingPath)) { // 如果基础包文件存在
      baseModuleIdMap = require(baseMappingPath);
      if (baseModuleIdMap.length != 0) { // 基础包文件有内容
        curModuleId = baseModuleIdMap[baseModuleIdMap.length - 1].id; // 取最后一个
      }
    }
  }
  ...
  const findPlatformItem = baseModuleIdMap.find((value) => { return value.path == pathRelative }); // 基础包模块集合
  const findBuzItem = buzModuleIdMap.find((value) => { return value.path == pathRelative }); // 业务包模块集合
  if (findPlatformItem) {
